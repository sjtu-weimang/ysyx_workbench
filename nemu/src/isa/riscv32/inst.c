...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R,
          R(rd) = ((uint64_t)((int64_t)((sword_t)src1) *
                              (int64_t)((sword_t)src2))) >>
                  32);
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, R,
          R(rd) = (((uint64_t)src1 * (uint64_t)src2)) >> 32);
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div, R,
          R(rd) = (src2 == 0) ? -1 : ((sword_t)src1 / (sword_t)src2));
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu, R,
          R(rd) = (src2 == 0) ? -1 : (src1 / src2));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai, I,
          R(rd) = ((sword_t)src1) >> (imm & ((1 << 6) - 1)));
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti, I,
          R(rd) = ((sword_t)src1 < imm) ? 1 : 0);
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, R(rd) = s->pc + 4;
          s->dnpc = s->pc + imm;
          IFDEF(CONFIG_FTRACE, ftrace_exec(s->pc, s->dnpc, rd, true)););
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I, R(rd) = s->pc + 4;
          s->dnpc = src1 + imm; s->dnpc -= (s->dnpc % 2);
          IFDEF(CONFIG_FTRACE, ftrace_exec(s->pc, s->dnpc, rd, false)););
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, N,
          IFDEF(CONFIG_ETRACE, etrace_exec(s->pc));
          s->dnpc = isa_raise_intr(17, s->pc));
