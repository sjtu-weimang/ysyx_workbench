...
void display_inst() {
  ...
  //   nbyte); char buf[128]; // 128 should be enough! char *p;
  // //   Statement("Most recently executed instructions");
  //   do {
  //     p = buf;
  //     p += sprintf(buf, "%s" FMT_WORD ": %08x ", (i+1)%MAX_IRINGBUF==end?"
  //     --> ":"     ", iringbuf[i].pc, iringbuf[i].inst); disassemble(p,
  //     buf+sizeof(buf)-p, iringbuf[i].pc, (uint8_t *)&iringbuf[i].inst, 4);
...
static void display_section_headers(int fd, Elf64_Ehdr eh,
                                    Elf64_Shdr sh_tbl[]) {
  // warn: C99
