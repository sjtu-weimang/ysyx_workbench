...
static void single_cycle(){
    ...
    dut.clk = 0;
    dut.eval();
    IFDEF(CONFIG_WTRACE, dump_single());
    dut.clk = 1;
    dut.eval();
}
...
void cpu_rst(int n) {
  ...
  while (n-- > 0)
    single_cycle();
  cpu.pc = 0x80000000;
  cpu.mstatus = 0x000001800;
}

void dpic_commit(uint32_t pc, uint32_t inst, uint32_t npc, char dm_access) {
  commit_cnt--;
  inst_cnt++;
  set_cpu();
  cpu.pc = npc;
  ...
  IFDEF(CONFIG_ITRACE, trace(pc, inst, g_print_step));
...
void cpu_exec(uint64_t n){
    ...
    switch(npc_state.state){
        ...
        case NPC_END:
        case NPC_ABORT:
          if (npc_state.halt_code != 0)
            dump();
          Log("npc: %s \n\thalt_code=%d\n\thalt_pc=" FMT_PADDR
              "\n\t%lu instructions executed\n\tIPC=%0.6f",
              npc_state.state == NPC_ABORT
                  ? ANSI_FMT("ABORT", ANSI_FG_RED)
                  : (npc_state.halt_code == 0
                         ? ANSI_FMT("HIT GOOD TRAP", ANSI_FG_GREEN)
                         : ANSI_FMT("HIT BAD TRAP", ANSI_FG_RED)),
              npc_state.halt_code, npc_state.halt_pc, inst_cnt,
