...
void instruction_ring_buffer_write() {
  for (int i = instruction_ring_buffer_head; i != instruction_ring_buffer_tail;
       (i == INSTRUCTION_LOG_BUF_SIZE - 1) ? i = 0 : i++) {
...
void instruction_ring_buffer_push(word_t code, word_t pc) {
  ...
  char log_buf[128];
  sprintf(log_buf, "0x%08x: %02x %02x %02x %02x\t%s", pc, code_buf[0],
          code_buf[1], code_buf[2], code_buf[3], disasm_buf);
  memcpy(instruction_ring_buffer[instruction_ring_buffer_tail], log_buf,
         sizeof(log_buf));
  int tail = instruction_ring_buffer_tail + 1;
  if (tail == INSTRUCTION_LOG_BUF_SIZE)
    tail = 0;
  if (tail == instruction_ring_buffer_head) {
    instruction_ring_buffer_head++;
    if (instruction_ring_buffer_head == INSTRUCTION_LOG_BUF_SIZE)
      instruction_ring_buffer_head = 0;
  }
...
void ftrace_init(const char *elf_file) {
  ...
  if (file == MAP_FAILED)
    panic("Failed to mmap %s", elf_file);
  ...
  for (int i = 0; i < header->e_shnum; i++) {
    if (sections[i].sh_type == SHT_SYMTAB) {
      Elf32_Sym *symtab =
          (Elf32_Sym *)((uint8_t *)file + sections[i].sh_offset);
      ...
      Log("Symbol table '%s' contains %d entries:",
          strtab_p + sections[i].sh_name, num_sym);
      for (int j = 0; j < num_sym; j++) {
        Elf32_Sym sym = symtab[j];
        if (ELF32_ST_TYPE(sym.st_info) == STT_FUNC) {
          Log("%d: %x, %x, %s", j, sym.st_value, sym.st_size,
              strtab_p + sym.st_name);
          func_cnt++;
        }
      }
    }
    ...
    if (sections[i].sh_type == SHT_SYMTAB) {
      ...
      for (int j = 0; j < num_sym; j++) {
        ...
        if (ELF32_ST_TYPE(sym.st_info) == STT_FUNC) {
          ...
          Log("%s : %x, %x", symbol_funcs[symbol_func_num].name,
              symbol_funcs[symbol_func_num].start_addr,
              symbol_funcs[symbol_func_num].end_addr);
          symbol_func_num++;
        }
      }
    }
...
void ftrace_exec(uint32_t pc_before, uint32_t pc_after, int rd, bool is_jal) {
  if (pc_before == 0x8000023c) {
    log_write("pc_before = %x, pc_after = %x, rd = %d, is_jal = %d", pc_before,
              pc_after, rd, is_jal);
  }
  ...
  int func_before = -1;
  for (int i = 0; i < symbol_func_num; i++) {
    if (pc_before >= symbol_funcs[i].start_addr &&
        pc_before < symbol_funcs[i].end_addr) {
      func_before = i;
      break;
    }
  }
  ...
  bool is_call = false;
  for (int i = 0; i < symbol_func_num; i++) {
    if (pc_after >= symbol_funcs[i].start_addr &&
        pc_after < symbol_funcs[i].end_addr) {
      func_after = i;
  ...
  if (func_before == func_after) {
    if (rd == 1) {
      log_write("%x: %*sFunc Call: %s to %s\n", pc_before, stack_depth, "",
                symbol_funcs[func_before].name, symbol_funcs[func_after].name);
      stack_depth++;
    } else if (rd == 0 && !is_jal) {
      if (stack_depth > 0)
        stack_depth--;
