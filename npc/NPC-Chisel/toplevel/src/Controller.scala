...
class Controller extends Module{
  ...
  val map = Array(
    ...
    CSRRW -> List(PCSelV.INC4,      ASelV.ZERO, BSelV.IMM, ImmType.I, WBSelV.NO_WB, 0.U, LdValue.INV, StValue.INV, ALUSelV.ZERO, CsrVal.RW),
    CSRRS -> List(PCSelV.INC4,      ASelV.ZERO, BSelV.IMM, ImmType.I, WBSelV.NO_WB, 0.U, LdValue.INV, StValue.INV, ALUSelV.ZERO, CsrVal.RS),
  )
  ...
  io.csr_rs_en := state===STATE_EXECUTE && signals(9)===CsrVal.RS
  io.csr_rw_en := state===STATE_EXECUTE && signals(9)===CsrVal.RW
  io.ecall_en := state===STATE_EXECUTE && signals(0)===PCSelV.ECALL
  io.mret_en :=  state===STATE_EXECUTE && signals(0)===PCSelV.MRET
}
