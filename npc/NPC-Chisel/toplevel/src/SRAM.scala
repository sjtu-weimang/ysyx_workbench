...
class SRAM extends Module{
    ...
    state_r := MuxLookup(state_r, state_r_idle, List(
        ...
        state_r_read_delay -> Mux(read_delay_cnt >= 1.U, state_r_wait_ready, state_r_read_delay),
    ...
    read_delay_cnt := Mux(state_r === state_r_read, read_delay_cnt + 1.U,0.U)
    ...
    state_w := MuxLookup(state_w, state_w_idle, List(
        ...
        state_w_write -> Mux(write_delay_cnt >= 1.U, state_w_write_ready, state_w_write_delay),
        state_w_write_delay -> Mux(write_delay_cnt >= 1.U, state_w_wait_ready, state_w_write_delay),
    ...
    val wstrb=RegInit(UInt(Constant.BitWidth),0.U)
    ...
    io.bresp:=0.U
    io.bvalid := state_w === state_w_write_ready

    pmem.io.valid:=state_r === state_r_read || state_w === state_w_write
    pmemio.raddr:=io.araddr
    pmem.io.waddr:=waddr
    pmem.io.wdata:=wdata
    pmem.io.wmask:=wstrb
