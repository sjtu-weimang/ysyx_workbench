...
class RegisterFile extends Module {
  ...
  val csr_valid =csr_id=/=7.U
  when(io.csr_rw_enable && csr_valid){
    registers(io.write_address) := csr(csr_id)
    csr(csr_id) := io.csr_wdata
  }.elsewhen(io.csr_rs_enable && csr_valid){
    registers(io.write_address) := csr(csr_id)
    csr(csr_id) := csr(csr_id) | io.csr_wdata
  }.elsewhen(io.csr_ecall_enable) {
    csr(2) := 11.U
    csr(1) := io.pc
    csr(0) := 0x1800.U;
  }.elsewhen(io.csr_mret_enable) {
    csr(0) := 0x80.U
  }
  registers(0):=0.U
  io.csr_ecall_ret:= csr(3)
  io.csr_mret_ret := csr(1)
  //io.csr_rdata := csr(csr_id)
